...
bool Stair::ShouldSkipWalkAnimation() const {
    if (m_object_id == static_cast<int>(AppUtil::StairId::STATIC_STAIR)) return false;
    return true;
}
    ...
    if (m_on_trigger) {
        ...
        if (it != AppUtil::GlobalObjectRegistry.end()) {
            ...
            LOG_INFO("Stair Activation: ID {}, Relation {}, IsRelative {}, Target ({}, {})", 
                     m_object_id, relation, m_isRelative, targetX, targetY);
            m_on_trigger(relation, m_isRelative, targetX, targetY);
        }
    }
}
