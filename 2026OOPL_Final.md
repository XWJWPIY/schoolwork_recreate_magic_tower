...
- (註：必殺與異常狀態之機率皆儲存於 `Enemy.csv` 的獨立欄位，由資料驅動機制於執行期載入，各怪物可各別設定不同數值，利於後續擴充維護)
...
2. **多種異質 CSV 讀取程式重複率過高與執行期屬性快取非同步開銷**
   - **問題**：為載入怪物、道具、商店等異質 CSV，專案初期各別撰寫讀取邏輯造成程式碼冗餘。同時，執行期頻繁的 `std::stoi` 與線性反查造成高頻傷害預估系統的效能隱患。為此我們引入了整數型快取，但卻衍生了「動態屬性更新（如商店交易次數）僅寫入原始字串 Map，而未同步更新快取」的致命 Bug，導致遊戲運行中交易次數在預估系統裡被判定為重置。
