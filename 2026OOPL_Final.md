...
- `Q` - 撤退（攻擊中使用）
- `L` - 開啟按鍵說明(再按一次關閉)
- `F` - 開啟電梯（和 8F 的老人互動獲得權限後可使用，但隱藏關卡無法使用。超級模式不受前述限制）
- `D` - 開啟心鏡（和 5F 的老人互動獲得權限後可使用，查看怪物的原始數值。超級模式不受前述限制）
- `G` - 切換超級長頸鹿模式提高攻擊、防禦力、金幣、經驗、體力、敏捷，可供高攻擊傷害與測試商店，並可直接使用電梯與心鏡 (作弊模式)
...
- 有（勇者敏捷）% 的機率閃避本次傷害（閃避成功則跳過以下判定，畫面顯示 MISS）
- 若敵人有「必殺攻擊」特殊能力，每次攻擊有 10% 機率直接擊殺勇者
- 若敵人有「衰弱」特殊能力，命中後有 9% 機率令勇者進入衰弱狀態
- 若敵人有「中毒」特殊能力，命中後有 9% 機率令勇者進入中毒狀態
- (註：必殺與異常狀態之機率皆儲存於 `Enemy.csv` 的獨立欄位，由資料驅動機制於執行期載入，各怪物可個別設定不同數值，利於後續擴充維護)
...
- 獲得指定道具「黃金色羽根」後，可按 `F` 鍵開啟。
- 使用上下箭頭選擇樓層
- 按下 `enter` 確認
- 一般模式下，只能在 20 樓以下使用，且傳送範圍最高只能到目前探索過的最高樓層
- 超級模式下，可以隨時使用，不受最高樓層、 20 樓、啟用權限的限制

#### 心鏡機制

- 獲得指定道具「心鏡」後，可按 `D` 鍵開啟怪物圖鑑。
- 使用左右箭頭切換頁面
- **無法破防判定**：若勇者攻擊力小於或等於怪物防禦力（我方攻擊 ≤ 敵方防禦），心鏡預估傷害將會直接顯示為 `???`。
- **確定性預估**：心鏡估算傷害時會排除閃避、必殺、衰弱/中毒狀態等隨機與機率性效果，僅依據雙方的 ATK、DEF、HP 以及段數進行確定性傷害加總預估。

#### 商店機制

**神像商店（地圖上固定位置）：**
- 貪婪之神：
    - 費用公式：用已交易次數 N 計算，購買費用 = 20 + N，超過第 25 次後每次額外再 +4
...
* **不參與繼承的獨立類別 (`class`，擁有或未擁有實例狀態的物件)：**
  ...
  * `BattleSystem` - 戰鬥演算類別，提供 `ProcessPlayerTurn` / `ProcessSingleEnemyHit` 等純邏輯靜態方法，將傷害計算與 UI 演出分離
...
1. **內建 `rand()` 隨機序列固定問題**
   ...
   - **解決方案**：淘汰舊式 `rand()`，改用 C++11 的 `<random>` 庫。使用 `std::high_resolution_clock` 作為非確定性種子，初始化 `std::mt19937` 偽隨機數產生器，並搭配 `std::uniform_int_distribution` 進行區間映射，確保每次遊戲啟動時的戰鬥與事件判定皆具備真正的不可預測性。

2. **多種異質 CSV 讀取程式重複率過高與執行期屬性快取非同步開銷**
   - **問題**：為載入怪物、道具、商店等異質 CSV，專案初期個別撰寫讀取邏輯造成程式碼冗餘。同時，執行期頻繁的 `std::stoi` 與線性反查造成高頻傷害預估系統的效能隱患。為此我們引入了整數型快取，但卻衍生了「動態屬性更新（如商店交易次數）僅寫入原始字串 Map，而未同步更新快取」的致命 Bug，導致遊戲運行中交易次數在預估系統裡被判定為重置。
   - **解決方案**：實作統一的 `CSVLoader` 類別進行底層封裝。優化欄位檢索機制，建立欄位索引與屬性 ID 的直接映射以消除 $O(N)$ 反查，並於載入時預解析屬性儲存至整數快取。針對動態修改屬性引發的快取失效，我們設計了 **Write-Through 雙向同步機制**（`ObjectMetadata::SetAttribute`），確保字串 Map 與整數快取在任何更新時均能 100% 同步，徹底解決了商店交易計數 Bug，實現資料高複用與零效能負擔。
...
在參與本次 OOP 物件導向程式設計實習之前，我的寫程式習慣多半偏向解決當前作業的「一次性代碼」，缺乏對軟體工程長遠發展的思量。然而這一次，是我首次嘗試以系統化的軟體架構來撰寫程式，開發一個極度注重物件間互動機制、統一資料管理、高擴充性與高維護性的中大型專案。在設計地圖物件與實體時，我耗費了許多心力反覆琢磨，只為了在「降低類似物件屬性的重複性」與「避免繼承體系過於冗長」之間取得最佳的平衡。這讓我深刻體會到，好的物件導向設計不僅是語法上的多型或封裝，更是如何透過解耦來劃分職責，讓系統的每一個微小分子都能自給自足，同時又能在大架構下協同運作。
